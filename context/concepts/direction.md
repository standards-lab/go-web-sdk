...
New implementations land in the `middleware` package: authentication and authorization enforcement
wait on an auth capability (`concepts/capability-middleware.md` holds the open placement question),
CORS waits on a browser client, and a recovery handler and a request ID wait for a service to need
them.

## Error mapping

The domain-error-to-status matchers that turn a returned error into a problem response. Additive to
the problem writers; needs a domain handler to exercise it.
...
`/readyz` attaches its `checks` extension member to an `about:blank` problem
(`design/problems.md`). A consumer that needs readiness failures under its own problem vocabulary
gets a type hook on `Readiness`, not an SDK-owned URI.
