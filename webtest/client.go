...
// the server's graceful shutdown waits for a connection that never sent a
// request, which every stop would then pay. A harness keeps one client per
// process, the runtime's own single-instance shape.
func NewClient(base string) *Client {
	return &Client{base: strings.TrimRight(base, "/"), http: newHTTPClient(processtest.Failsafe)}
}
...
// status, as the web package writes one, and returns it.
